...
for i in {1..5}; do getent hosts ${FLINK_JM_ADDR} && break || echo "Cannot reach jobmanager at hostname ${FLINK_JM_ADDR}, will retry" && sleep 5; done
...
exec "${FLINK_BIN_DIR}"/start-common.sh taskmanager "${args[@]}"
